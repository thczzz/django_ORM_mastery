    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            with transaction.atomic():
                ...
                # customer.objects.select_for_update().filter(name=x).update(balance=F('balance') - z)
                # customer.objects.select_for_update().filter(name=y).update(balance=F('balance') + z)
